...
def find_rule_files():
    """Find all yaml rule files under C# rules directory."""
    pattern = os.path.join(RULES_ROOT, "**", "*.yaml")
    return sorted(glob.glob(pattern, recursive=True))

CSHARP_RULE_FILES = find_rule_files()
...
class TestCSharpRulesIntegrity:

    def test_rules_directory_is_not_empty(self):
        """Ensure C# rules exist."""
        assert len(CSHARP_RULE_FILES) > 0, "No C# rule YAML files found"

    def test_semgrep_cli_validates_csharp_rules(self):
        """Runs `semgrep --validate --config rules/` across C# rules."""
        ...
        assert result.returncode == 0, (
            f"Global Semgrep C# rule validation failed!\n"
            f"STDOUT:\n{result.stdout}\n"
            f"STDERR:\n{result.stderr}"
        )
    ...
    def test_rule_metadata_and_schema(self, rule_file):
        ...
        for rule in data["rules"]:
            ...
            assert rule_id.startswith("csharp-"), f"Rule ID '{rule_id}' should start with 'csharp-'"
            assert "message" in rule and len(rule["message"].strip()) > 0, f"Rule {rule_id} missing message"
            assert "languages" in rule and "csharp" in rule["languages"], f"Rule {rule_id} missing 'csharp' language"
    ...
    def test_rule_has_matching_test_fixture(self, rule_file):
        """Verify that every C# rule has a corresponding .cs test fixture."""
        base, _ = os.path.splitext(rule_file)
        cs_fixture = f"{base}.cs"
        assert os.path.exists(cs_fixture), f"Missing test fixture for {rule_file}. Expected '{base}.cs'"
